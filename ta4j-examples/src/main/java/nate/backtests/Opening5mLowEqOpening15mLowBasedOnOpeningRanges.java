...
public class Opening5mLowEqOpening15mLowBasedOnOpeningRanges extends BackTestWithDailyMgi {
...
//    private static final List<Double> stopLossExtPercentList = Arrays.asList(0.5);
    ...
    private static void createRulesAndRunBackTest(BarSeries series, Double takeProfitsExtensionPercent, Double stopLossExtensionPercent, Double retracePercent) {
        ...
        // Sell Rule
        Rule sellingRule = new RangeExtensionHitRule(series, Range.OPENING_15_MIN, takeProfitsExtensionPercent, true)
                .or(new OutsideOfMarketTimeRangeRule(series, MarketTime.RTH_0945, MarketTime.RTH_1500)).or(new RangeExtensionHitRule(series, Range.OPENING_15_MIN, stopLossExtensionPercent, false));
        String sellingRuleStr = "(TP: " + DoubleFormatter.formatPercent(takeProfitsExtensionPercent)
                + " Extension Higher of O15m Extension Hit) OR (SL: " + DoubleFormatter.formatPercent(stopLossExtensionPercent) + " Extension Lower of O15m Extension Hit ) OR (Outside of 0945-1500)";

        buyingRule = new DailyTradeNotTakenRule().and(new MarketTimeRangeRule(series, MarketTime.RTH_0945, MarketTime.RTH_1500))
                .and(new RangeHighLowEqualsRange(series, Range.OPENING_5_MIN, Range.OPENING_15_MIN, false))
                .and(new RangeRetracementHitRule(series, Range.OPENING_15_MIN, retracePercent, true));
        String buyingRuleStr = "(Daily trade not taken) AND (Between 0945-1500) AND (O5m Range Low = O15m Range Low) AND (" + DoubleFormatter.formatPercent(retracePercent) +  " Retrace of O15m Range)";

        run(series, buyingRule, sellingRule, buyingRuleStr, sellingRuleStr, Trade.TradeType.BUY);
//        System.out.println("");
    }
}
